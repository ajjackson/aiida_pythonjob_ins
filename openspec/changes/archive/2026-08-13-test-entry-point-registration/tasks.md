...
- [x] 1.1 Add a test module covering plugin registration, with one parametrised
      case per registered class pairing its documented entry-point name with the
      class the factory must return.
- [x] 1.2 Cover the data types through AiiDA's data factory, asserting the
      returned object is the expected class rather than merely resolving.
- [x] 1.3 Cover the workflows through AiiDA's workflow factory, on the same
      ...
      path.
- [x] 1.4 Concisely note in the module docstring that entry-point metadata is written at
...
- [x] 2.1 Temporarily rename one data entry point in `pyproject.toml`, reinstall,
      and confirm the corresponding case fails while the rest of the suite stays
      green; restore afterwards.
- [x] 2.2 Repeat for one workflow entry point by removing it, confirming the case
      fails - this is the hole that motivated the change, so verify it directly
      rather than assuming symmetry with the data case.
- [x] 2.3 Confirm the checks need no profile, stored node or submitted job, as the
      spec requires.

## 3. Close out

- [x] 3.1 Run the full suite and `ruff check`; confirm no regression and no
      measurable increase in runtime.
- [x] 3.2 Mark the entry-point sub-item of `document-poc-baseline` design item 6
      as addressed by this change, leaving the other three gaps deferred.
- [x] 3.3 Archive this change after `document-poc-baseline` is archived, since the
