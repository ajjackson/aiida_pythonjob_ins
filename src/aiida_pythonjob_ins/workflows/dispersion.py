...
@calcfunction
def generate_band_path(
    force_constants: ForceConstantsData, q_spacing: Float
) -> KpointsData:
    """Build a seekpath high-symmetry q-point path as a native ``KpointsData``.

    A parent-side ``calcfunction`` (not a PythonJob): path-building is cheap and
    needs a loaded profile to construct the ``KpointsData`` node, which a remote
    PythonJob subprocess could not do.
    """
    fc = force_constants.get_force_constants()
    qpts, labels = band_path_qpoints(fc, q_spacing.value)
    cell = fc.crystal.cell_vectors.to("angstrom").magnitude
    return qpoints_to_kpoints_data(qpts, cell, labels=labels)
...
class DispersionWorkChain(WorkChain):
    ...
    def generate_path(self):
        """Step 2: build the seekpath q-point path (parent-side calcfunction)."""
        if not self.ctx.read.is_finished_ok:
            return self.exit_codes.ERROR_SUB_PROCESS_FAILED

        # calcfunctions run synchronously and return their output node directly.
        self.ctx.path = generate_band_path(
            self.ctx.read.outputs.result, self.inputs.q_spacing
        )
        return None
